...
class CodeASTEnc(nn.Module):
    def __init__(self,
        ...
        for _ in range(6):
            self.hgt_enc_ops.append(GeneralConv("hgt", 512, 512, 3, 5, 8, drop_rate,
    ...
    def forward(self, data):
        ...
        graph = data['graph']
        # graph_batch = data.x_batch_dict['graph'] # 它就是个tensor([0, 1, 2, 3, 4]什么的，并没有确切的属于哪个数据。
        ...
        #！！！残差链接
        # for hgt, relu, norm in zip(self.hgt_enc_ops, self.hgt_relu_ops, self.hgt_norm_ops):
        #     node_inp_ = hgt(node_inp, node_type, edge_index, edge_type)  # dict(xx_node:[batch_xx_node_num,hid_dims])
        #     node_inp = norm(node_inp.add(relu(node_inp_)))
        ...
        if self.train_mode:  # 如果进行训练
            ...
            for i in range(0, self.max_big_epochs):
                ...
                if i + 1 >= self.start_valid_epoch:
                    ...
                    #                                    last=False)  # 根据验证集loss选择best_net
                    # 每次都测试一下
                    temp_test_eval_df = self.eval(test_srcs=test_data['code_asts'],
                    ...
                    for i in range(0, len(temp_test_eval_df.columns), 4):
                        print(temp_test_eval_df.iloc[:, i:i + 4])
                ...
                if self.beam_width == 1:
                    for i in range(self.text_max_len + 1):  # 每步开启
                        pred_out = dec_op(ast_enc=batch_ast_enc, ast_code_enc=batch_ast_code_enc,
                                          ...
                                          text_input=batch_text_input, node_inp=node_inp)  # 预测该步输出 (B,text_voc_size,L_text)
