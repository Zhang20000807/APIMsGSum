...
from my_lib.neural_model.pyhgt.conv import GeneralConv
...
class Datasetx(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['node', 'sibling_next', 'node'].edge_index = torch.tensor(
            self.code_asts[index]['sibling_next_sibling_edges']).long()
        data['node', 'sibling_prev', 'node'].edge_index = torch.tensor(
            self.code_asts[index]['sibling_prev_sibling_edges']).long()
        ...
        data["graph"].x = self.graphs[index]['node_inp']
        data["graph"].node_type = torch.tensor(self.graphs[index]['node_type']).long().to('cuda')
        # data["graph"].edge_index = torch.tensor(self.graphs[index]['edge_index']).long().to('cuda')
        # data["graph"].edge_type = torch.tensor(self.graphs[index]['edge_type']).long().to('cuda')
        edge_index_1 = self.graphs[index]['edge_index'][0] + self.graphs[index]['edge_index'][1]
        edge_index_2 = self.graphs[index]['edge_index'][1] + self.graphs[index]['edge_index'][0]
        edge_index = [edge_index_1, edge_index_2]
        edge_type = self.graphs[index]['edge_type'] + self.graphs[index]['edge_type']
        data["graph"].edge_index = torch.tensor(edge_index).long().to('cuda')
        data["graph"].edge_type = torch.tensor(edge_type).long().to('cuda')
...
class CodeASTEnc(nn.Module):
    def __init__(self,
        ...
        # hgt_enc#new# microsoft/codebert-base或者huggingface/CodeBERTa-small-v1
        model_folder = "model_folder"
        self.cb_tokenizer = AutoTokenizer.from_pretrained(model_folder, local_files_only=True)
        # self.cb_tokenizer = AutoTokenizer.from_pretrained("microsoft/codebert-base")
        # self.cb_model = AutoModel.from_pretrained("huggingface/CodeBERTa-small-v1").to('cuda')
        # self.fc = nn.Linear(768, 512).to('cuda')
        # self.embedding_layer = nn.Embedding(50265, 512, padding_idx=kwargs['pad_idx'])
        # self.lstm_layer = nn.LSTM(512, 512, 1, batch_first=True)
        self.embedding_layer = nn.Embedding(len(self.cb_tokenizer.vocab), 512, padding_idx=kwargs['pad_idx'])
        nn.init.xavier_uniform_(self.embedding_layer.weight[1:, ])
        # hgt_enc#new#
        self.hgt_enc_ops = nn.ModuleList()
        self.hgt_relu_ops = nn.ModuleList()
        self.hgt_norm_ops = nn.ModuleList()
        for _ in range(6):
            self.hgt_enc_ops.append(GeneralConv("hgt", 512, 512, 4, 5, 8, drop_rate,
                                                use_norm=True, use_RTE=False))
            self.hgt_relu_ops.append(nn.Sequential(nn.ReLU(), nn.Dropout(p=drop_rate)))
            self.hgt_norm_ops.append(nn.LayerNorm(512).to('cuda'))
        ...
        for _ in range(ast_gnn_layers):
            if ast_GNN == TransformerConv:
                gnn = HeteroConv({
                    ('node', 'child', 'node'): TransformerConv((emb_dims, emb_dims),
                                                               out_channels=emb_dims // code_att_heads,
                                                               heads=code_att_heads, aggr=ast_gnn_aggr,
                                                               dropout=drop_rate, root_weight=True),
                    ('node', 'parent', 'node'): TransformerConv((emb_dims, emb_dims),
                                                                out_channels=emb_dims // code_att_heads,
                                                                heads=code_att_heads, aggr=ast_gnn_aggr,
                                                                dropout=drop_rate, root_weight=False),
                    ('node', 'sibling_next', 'node'): TransformerConv((emb_dims, emb_dims),
                                                                      out_channels=emb_dims // code_att_heads,
                                                                      heads=code_att_heads, aggr=ast_gnn_aggr,
                                                                      dropout=drop_rate, root_weight=False),
                    ('node', 'sibling_prev', 'node'): TransformerConv((emb_dims, emb_dims),
                                                                      out_channels=emb_dims // code_att_heads,
                                                                      heads=code_att_heads, aggr=ast_gnn_aggr,
                                                                      dropout=drop_rate, root_weight=False),
                    ('node', 'dfg_next', 'node'): TransformerConv((emb_dims, emb_dims),
                                                                  out_channels=emb_dims // code_att_heads,
                                                                  heads=code_att_heads, aggr=ast_gnn_aggr,
                                                                  dropout=drop_rate, root_weight=False),
                    ('node', 'dfg_prev', 'node'): TransformerConv((emb_dims, emb_dims),
                                                                  out_channels=emb_dims // code_att_heads,
                                                                  heads=code_att_heads, aggr=ast_gnn_aggr,
                                                                  dropout=drop_rate, root_weight=False),
                }, aggr='sum')
                ...
                gnn = HeteroConv({
                    ('node', 'child', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                       root_weight=True),
                    ('node', 'parent', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                        root_weight=False),
                    ('node', 'sibling_next', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                              root_weight=False),
                    ('node', 'sibling_prev', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                              root_weight=False),
                    ('node', 'dfg_next', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                          root_weight=False),
                    ('node', 'dfg_prev', 'node'): ast_GNN((emb_dims, emb_dims), emb_dims, aggr=ast_gnn_aggr,
                                                          root_weight=False),
                }, aggr='sum')
            else:
                gnn = HeteroConv({
    ...
    def forward(self, data):
        ...
        # new#
        graph_embedding = []
        # batch_num = len(data["graph"].x)
        graph_batch_data = []
        for idx, graph_batch in enumerate(data["graph"].x):
            embedding = []  # embedding存储的一个图中的所有节点的向量
            for item in graph_batch:
                pattern = r'[a-zA-Z]+'
                tokens = re.findall(pattern, item)[:128]
                item_str = ' '.join(tokens).lower()
                # print("NODE:", item_str)
                inputs = self.cb_tokenizer(item_str, return_tensors="pt").to('cuda')
                inputs = inputs['input_ids']

                # 普通的embedding层
                test_embeddings = self.embedding_layer(inputs)
                test_embeddings = torch.squeeze(test_embeddings, dim=0)
                item_embed = test_embeddings.sum(dim=0)
                # print("SUM",item_embed.shape)

                embedding.append(item_embed)

            temp_length = len(embedding)
            # print(temp_length)
            graph_batch_data.append((idx, temp_length))
            embedding = torch.stack(embedding)
            graph_embedding.append(embedding)
        graph_embedding = torch.cat(graph_embedding, dim=0)
        # print(graph_embedding.shape)
        data["graph"].x = graph_embedding
        graph = data['graph']

        graph_batch = torch.tensor([value for value, count in graph_batch_data for _ in range(count)]).to('cuda')

        node_inp, node_type, edge_index, edge_type = graph['x'], graph['node_type'], graph[
            'edge_index'], graph['edge_type']

        for hgt, relu, norm in zip(self.hgt_enc_ops, self.hgt_relu_ops, self.hgt_norm_ops):
            node_inp = hgt(node_inp, node_type, edge_index, edge_type)  # dict(xx_node:[batch_xx_node_num,hid_dims])
            node_inp = norm(node_inp.add(relu(node_inp)))
            # node_inp = norm(node_inp)

        node_inp, _ = to_dense_batch(node_inp, batch=graph_batch, fill_value=self.pad_idx, max_num_nodes=2150)
        ...
        code_emb = data['node'].x[data['node'].code_mask == True, :] * np.sqrt(
            self.emb_dims)  ##[batch_leaf_node_num,emb_dims]
        # data['node'].x=self.ast_emb_norm_op(data['node'].x) ##[batch_ast_node_num,emb_dims]
        code_mpos_emb = self.code_mpos_emb_op(
            data['node'].code_pos[0, :][data['node'].code_mask == True])  # [batch_leaf_node_num,emb_dims]
        code_npos_emb = self.code_npos_emb_op(
            data['node'].code_pos[1, :][data['node'].code_mask == True])  # [batch_leaf_node_num,emb_dims]
        ...
        sparse_code_enc = code_enc.contiguous().view(-1, code_enc.size(-1))[code_mask.view(-1) == True,
                          :]  ###[batch_leaf_node_num,emb_dims] convert dense batch into sparse batch
        data['node'].x[data['node'].code_mask == True, :] = data['node'].x[data['node'].code_mask == True, :].add(
            sparse_code_enc)  # [batch_leaf_node_num,emb_dims]
        ...
        ast_code_enc, _ = to_dense_batch(data.x_dict['node'][data['node'].code_mask == True],
                                         ...
                                         max_num_nodes=self.code_max_len)  # [batch_data_num,code_max_len]
        # print("OK1")
        return ast_enc, ast_code_enc, code_enc, code_src_map, node_inp
...
class Dec(nn.Module):
    def __init__(self,
        ...
        self.text_dec_op = DualTranDec(query_dims=emb_dims,
                                       ...
                                       mode='sequential',
                                       self_causality=True)
        self.graph_dec_op = DualTranDec(query_dims=emb_dims,  # 512
                                        key_dims=enc_out_dims,  # 512
                                        head_num=att_heads,  # 8
                                        ff_hid_dims=ff_hid_dims,  # 2048
                                        head_dims=att_head_dims,  # None
                                        layer_num=6,  # 6
                                        drop_rate=drop_rate,  # 0.2
                                        pad_idx=kwargs['pad_idx'],
                                        mode='sequential',
                                        self_causality=True)
        self.classification_fc = nn.Linear((text_max_len + 1) * 2 * emb_dims, 1)
        self.sigmoid = nn.Sigmoid()
        self.attention = nn.Softmax(dim=2).to('cuda')
    ...
    def forward(self, ast_enc, ast_code_enc, code_enc, code_src_map, text_input, node_inp):
        ...
        graph_mask = node_inp.abs().sum(-1).sign()
        text_dec_old = self.text_dec_op(query=text_dec,  # torch.Size([4, 39, 512])
                                        key1=ast_enc,  # torch.Size([4, 522, 512])
                                        key2=code_enc,  # torch.Size([4, 321, 512])
                                        query_mask=text_mask,  # torch.Size([4, 39])
                                        key_mask1=ast_mask,  # torch.Size([4, 522])
                                        key_mask2=code_mask  # torch.Size([4, 321])
                                        )  # (B,L_text,D_text_emb)
        text_dec_new = self.graph_dec_op(query=text_dec,  # torch.Size([4, 39, 512])
                                         key1=node_inp,  # torch.Size([4, 512, 512])
                                         key2=code_enc,  # torch.Size([4, 321, 512])
                                         query_mask=text_mask,  # torch.Size([4, 39])
                                         key_mask1=graph_mask,  # torch.Size([4, 512])
                                         key_mask2=code_mask  # torch.Size([4, 321])
                                         )  # (B,L_text,D_text_emb)

        # # 计算注意力权重
        # weights = self.attention(text_dec_old)
        # # 将注意力权重应用于第二个张量
        # weighted_tensor2 = text_dec_new * weights
        # # 将两个张量相加
        # text_dec = text_dec_old + weighted_tensor2

        # text_dec = text_dec_old + text_dec_new

        # 拆解
        batch_size, seq_len, embedding_dim = text_dec_old.shape
        temp = torch.cat([text_dec_old, text_dec_new], dim=2)
        text_dec_flat = temp.view(batch_size, seq_len * 2 * embedding_dim)

        # 分类
        classification_result_flat = self.classification_fc(text_dec_flat)
        # 将分类结果重新变形成(batch_size, text_len)的张量
        classification_result = classification_result_flat.view(batch_size, 1)
        classification_result = self.sigmoid(classification_result)
        classification_result[classification_result < 0.5] = 0
        classification_result[classification_result > 0.5] = 1
        classification_result = classification_result.squeeze(1)

        weights = self.attention(text_dec_old)
        weighted_tensor2 = text_dec_new * weights
        text_dec_id = text_dec_old + weighted_tensor2
        # print("1", text_dec_idx.shape)
        # 赋值
        for idx in range(len(classification_result)):
            if classification_result[idx].item() == 1:
                text_dec_idx = text_dec_id[idx]
            else:
                text_dec_idx = text_dec_old[idx]
            text_dec_idx = text_dec_idx.unsqueeze(0)
            if idx == 0:
                text_dec = text_dec_idx
            else:
                text_dec = torch.cat((text_dec, text_dec_idx), dim=0)
        # print("OK2")
        if not self._copy:
...
class TNet(BaseNet):
    ...
    def forward(self, code_ast):
        ...
        del code_ast['text']
        ast_enc, ast_code_enc, code_enc, code_src_map, node_inp = self.enc_op(data=code_ast)
        text_output = self.dec_op(ast_enc=ast_enc, ast_code_enc=ast_code_enc, code_enc=code_enc,
                                  code_src_map=code_src_map,
                                  text_input=text_input, node_inp=node_inp)
        return text_output
...
class TModel(TransSeq2Seq):
    ...
    def _logging_paramerter_num(self):
        ...
        code_ast_enc_param_num = sum(
            x.numel() for x in self.net.module.enc_op.code_enc_op.parameters() if x.requires_grad) + \
                                 sum(x.numel() for x in self.net.module.enc_op.gnn_ops.parameters() if
                                     x.requires_grad) + \
    ...
    def fit(self,
        ...
        self.text_max_len = 0
        for code_ast, text, graph in zip(train_data['code_asts'], train_data['texts'], train_data['graph']):
            ...
            self.graph_max_size = max(self.graph_max_size, len(graph['node_inp']))
        ...
        train_code_asts, train_texts, train_ids, train_graph = zip(
            *random.sample(
                list(zip(train_data['code_asts'], train_data['texts'], train_data['ids'], train_data['graph'])),
                min(self.max_train_size,
                    len(train_data['code_asts']))
            )
        )
        # print("ids:", train_ids[0])
        # print("train_code_asts:", train_code_asts[0])
        # print("train_texts:", train_texts[0])
            ...
            for i in range(0, self.max_big_epochs):
                ...
                if i + 1 >= self.start_valid_epoch:
                    self.max_valid_size = len(
                        valid_data['code_asts']) if self.max_valid_size == -1 else self.max_valid_size
                    valid_srcs, valid_graph, valid_tgts, ex_text_i2ws = zip(
                        *random.sample(list(zip(valid_data['code_asts'],
                                                valid_data['graph'],
                                                valid_data['texts'],
                                                valid_data['text_dic'][
                                                    'ex_text_i2ws'])),
    ...
    def predict(self,
        ...
        with torch.no_grad():  # 取消梯度
            ...
            for batch_data in pbar:
                ...
                # 先跑encoder，生成编码
                batch_ast_enc, batch_ast_code_enc, batch_code_enc, batch_code_src_map, node_inp = enc_op(batch_data)
                batch_text_output: list = []  # 每步的output tensor
                if self.beam_width == 1:
                    for i in range(self.text_max_len + 1):  # 每步开启
                        pred_out = dec_op(ast_enc=batch_ast_enc, ast_code_enc=batch_ast_code_enc,
                                          code_enc=batch_code_enc, code_src_map=batch_code_src_map,
                                          text_input=batch_text_input,
                                          node_inp=node_inp)  # 预测该步输出 (B,text_voc_size,L_text)
    ...
    def generate_texts(self, code_asts, graph, text_dic, res_path, gold_texts, raw_data, token_data, **kwargs):
        '''
        生成src对应的tgt并保存
        :param code_asts:
        :param text_dic:
...
if __name__ == '__main__':
    ...
    logging.info(train_avail_data_path)
    ...
    # train_data['code_asts'] = train_data['code_asts'][:dataset_size]
    # train_data['texts'] = train_data['texts'][:dataset_size]
    # train_data['ids'] = train_data['ids'][:dataset_size]
    # train_data['graph'] = train_data['graph'][:dataset_size]
    # test_data['code_asts'] = test_data['code_asts'][:dataset_size]
    # test_data['texts'] = test_data['texts'][:dataset_size]
    # test_data['ids'] = test_data['ids'][:dataset_size]
    # test_data['graph'] = test_data['graph'][:dataset_size]
